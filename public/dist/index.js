...
const calcTotal = (arr) => {
    let total = 0;
    arr.forEach((product) => total += product.price);
...
const createList = () => {
    ...
    const jsonParsedBasket = stringBasket ? JSON.parse(stringBasket) : [];
    ...
    uniqueArrSorted.forEach((product, index) => {
...
$(document).ready(() => {
    ...
    $('body').on('click', '.shoppingmenu-list-item-bin', (event) => {
        ...
        const basketParsed = basketString ? JSON.parse(basketString) : [];
        const plant = trashCan.parent().prev().children().first().text();
        const filteredBasket = basketParsed.filter((x) => x.item !== plant);
    ...
    $('body').on('click', '.shoppingmenu-list-item-buttons-up', (event) => {
        ...
        const product = { item: plant, price: Number(price) };
